require("lazyload").on_vim_enter(function()
  ...
  --
  -- h/t: https://git.cryptomilk.org/users/asn/dotfiles.git/tree/dot_config/nvim/lua/plugins/pack-ui.lua
  --
  ...
  local function build_content()
    ...
    local function render_plugin(p, icon, hl_group)
      ...
      local unreleased_str = unreleased
        ...
        or ""
      local line =
        string.format("   %s %s%s%s%s%s", icon, name, pad, ver_display, update_str, unreleased_str)
      ...
      if state.expanded[name] then
        ...
        if unrel and #unrel > 0 then
          add(
            string.format("     ⚠ %d breaking change(s) unreleased on main:", #unrel),
            "PackUiBreaking"
          )
          line_to_plugin[#lines] = name
  ...
  local function setup_keymaps()
    ...
    vim.keymap.set("n", "D", function()
      ...
      if #pdata > 0 and pdata[1].active then
        vim.notify(
          string.format("vim.pack: %s is active, remove from config first", name),
          vim.log.levels.WARN
        )
        return
      end
      ...
      local choice =
        vim.fn.confirm(string.format("Delete plugin %s?", name), "&Yes\n&No", 2, "Question")
      if choice == 1 then
        close()
